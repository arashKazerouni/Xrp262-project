...
function CopyAddress({ value }: { value: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 text-[10px] text-slate-600 group-hover:text-blue-400">{copied ? "COPIED" : "COPY"}</span>
    </button>
  );
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div className="mb-3 font-mono text-[10px] font-semibold tracking-[0.2em] text-blue-400">{children}</div>;
}

function Arrow() {
  return <div className="hidden h-px flex-1 bg-gradient-to-r from-blue-500/30 via-blue-500/70 to-blue-500/20 md:block" />;
}
...
export default function Home() {
  return (
    <main className="relative overflow-hidden">
      ...
      <section className="mx-auto grid min-h-[720px] max-w-[1280px] items-center gap-16 px-4 py-24 sm:px-6 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:py-28">
        <div>
          ...
          <div className="mt-12 grid max-w-xl grid-cols-2 gap-6 border-t border-slate-800/80 pt-6 sm:grid-cols-4">
            ...
            <div><div className="font-mono text-lg text-white">899.57B</div><div className="mt-1 text-[10px] uppercase tracking-widest text-slate-600">Circulating</div></div>
            <div><div className="font-mono text-lg text-white">ACTIVE</div><div className="mt-1 text-[10px] uppercase tracking-widest text-slate-600">Registry</div></div>
            <div><div className="font-mono text-lg text-white">SAC</div><div className="mt-1 text-[10px] uppercase tracking-widest text-slate-600">Asset layer</div></div>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="card blue-line relative rounded-xl p-5 sm:p-7">
            <div className="mb-7 flex items-center justify-between">
              <div className="font-mono text-[10px] tracking-[0.18em] text-slate-500">LIVE ARCHITECTURE SIGNAL</div>
              <div className="flex items-center gap-2 font-mono text-[9px] text-blue-400"><span className="h-1.5 w-1.5 rounded-full bg-blue-400 pulse" /> VERIFIED MODEL</div>
            </div>
            {[
              ...
              <div key={name}>
                ...
                {i < 2 && <div className="mx-auto h-7 w-px bg-gradient-to-b from-blue-500/60 to-transparent" />}
              </div>
            ))}
      ...
      <section id="architecture" className="border-y border-slate-900 bg-[#0a0e17]/55">
        <div className="mx-auto max-w-[1280px] px-4 py-24 sm:px-6 lg:px-8">
          <SectionLabel>01 / ARCHITECTURE</SectionLabel>
          <div className="max-w-2xl">
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Tri-tier asset architecture.</h2>
            <p className="mt-4 text-sm leading-7 text-slate-500">A classic Stellar asset, its native Soroban Asset Contract, and the LEXORA controller form the protocol surface.</p>
          </div>
          <div className="mt-12 grid gap-px overflow-hidden rounded-xl border border-slate-800 bg-slate-800 md:grid-cols-3">
            {[
              ["01", "Classic Asset", "Issuer-defined Stellar asset", "Native ledger layer"],
              ["02", "SAC", "Stellar Asset Contract", "Programmable asset interface"],
              ["03", "LEXORA", "Protocol controller", "Policy and supply controls"],
            ].map(([n,t,d,s]) => (
              <div key={n} className="bg-[#0b101b] p-6 sm:p-8">
                ...
                <div className="mt-12 border-t border-slate-800 pt-4 font-mono text-[10px] uppercase tracking-wider text-slate-600">{s}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="state" className="mx-auto max-w-[1280px] px-4 py-24 sm:px-6 lg:px-8">
        <SectionLabel>02 / VERIFIED STATE</SectionLabel>
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div><h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Protocol telemetry.</h2><p className="mt-4 text-sm text-slate-500">Genesis state verified against XRP262 mainnet control records.</p></div>
          <span className="font-mono text-[10px] text-slate-600">PRECISION / STANDARD</span>
        </div>
      ...
      <section id="supply" className="border-y border-slate-900 bg-[#0a0e17]/55">
        <div className="mx-auto max-w-[1280px] px-4 py-24 sm:px-6 lg:px-8">
          <SectionLabel>03 / SUPPLY ARCHITECTURE</SectionLabel>
          <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Supply with an explicit invariant.</h2>
          <div className="mt-12 grid gap-4 lg:grid-cols-[1fr_auto_1fr_auto_1fr] lg:items-center">
            ...
            )).flatMap((node, i, arr) => i < arr.length - 1 ? [node, <Arrow key={`a${i}`} />] : [node])}
          </div>
          <div className="mt-8 rounded-lg border border-blue-500/20 bg-blue-500/[0.035] px-5 py-4 font-mono text-xs text-slate-400">
            <span className="text-blue-400">INVARIANT</span> · Circulating = Genesis Minted − Total Burned
          </div>
        </div>
      </section>

      <section id="verification" className="mx-auto max-w-[1280px] px-4 py-24 sm:px-6 lg:px-8">
        <SectionLabel>04 / VERIFICATION</SectionLabel>
        <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
          <div>
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Verify the state yourself.</h2>
            <p className="mt-4 max-w-lg text-sm leading-7 text-slate-500">The interface is a view into public protocol state. Addresses and ledger state can be independently checked through Stellar infrastructure.</p>
          </div>
          <div className="space-y-3">
            {[
              ...
              <div key={label} className="card flex flex-col gap-3 rounded-lg p-5 sm:flex-row sm:items-center sm:justify-between">
                <div><div className="font-mono text-[9px] tracking-[0.16em] text-blue-400">{label}</div><div className="mt-2 text-xs text-slate-600">MAINNET / PUBLIC</div></div>
